...
export class UsageTracker {
  ...
  update(prompt: unknown, completion: unknown, cachedRead?: unknown, cachedWrite?: unknown): Extract<ProviderEvent, { type: "usage" }> | undefined {
    ...
    if (p === this.prompt && c === this.completion && r === this.cachedRead && w === this.cachedWrite) return undefined;
    const event = { type: "usage" as const, promptTokens: p - this.prompt, completionTokens: c - this.completion, promptTokensTotal: p, completionTokensTotal: c, ...(r > this.cachedRead ? { cachedReadTokens: r - this.cachedRead } : {}), ...(w > this.cachedWrite ? { cachedWriteTokens: w - this.cachedWrite } : {}) };
    this.prompt = p;
    this.completion = c;
    this.cachedRead = r;
    this.cachedWrite = w;
